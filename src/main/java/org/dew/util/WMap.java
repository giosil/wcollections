...
class WMap implements Map
{
  ...
  public WMap(HttpServletRequest request)
  {
    map = new MapRequest(request);
  }
  
  public WMap(HttpServletRequest request, boolean boCreateSession)
  {
    map = new MapSession(request.getSession(boCreateSession));
  }
  
  public WMap(HttpSession session)
  {
    map = new MapSession(session);
  }
  
  public static
  WMap getParameters(HttpServletRequest request)
  {
    Map map = new HashMap();
    Enumeration enumeration = request.getParameterNames();
    while(enumeration.hasMoreElements()) {
      String sParameterName = (String) enumeration.nextElement();
      map.put(sParameterName, request.getParameter(sParameterName));
    }
    return new WMap(map);
  }
  
  public static
  WMap getConfig(ServletConfig servletConfig)
  {
    Map map = new HashMap();
    Enumeration enumeration = servletConfig.getInitParameterNames();
    while(enumeration.hasMoreElements()) {
      String sParameterName = (String) enumeration.nextElement();
      map.put(sParameterName, servletConfig.getInitParameter(sParameterName));
    }
    return new WMap(map);
  }
  
  public static
  WMap getSession(HttpServletRequest request)
  {
    return new WMap(request.getSession());
  }
  
  public static
  WMap getSession(HttpServletRequest request, boolean boCreateSession)
  {
    return new WMap(request, boCreateSession);
  }
  
  public static
  WMap getAttributes(HttpServletRequest request)
  {
    return new WMap(request);
  }
  ...
  public WList getSequence(Object key) {
    String sKey = key.toString();
    List listSequence = new ArrayList();
    for(int i = 0; i < Integer.MAX_VALUE; i++) {
      Object oValue = get(sKey + i);
      if(oValue == null) break;
      listSequence.add(oValue);
    }
    return new WList(listSequence);
  }
